...
function map() {
    ...
    option = {
        ...
        tooltip: {
            trigger: 'item',
            formatter: (params,ticket,fn) =>　{
                // console.log(22222,params,ticket)
                return `<span style="color:#fff">${params.name}：${params.value[2]}</span>`
            }
        },
        geo: {
            map: 'china',
    ...
    let zoom_dis = 1
    myChart.on('dataZoom', function (params) {//选取的x轴值
    ...
    myChart.setOption({ series: [{ symbolSize: (0 | 2 / zoom_dis * 2) }] })
