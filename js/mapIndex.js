function initMap() {
    ...
    // 分配颜色
    let maxValue = mapData.reduce((a, b) => a.value > b.value ? a : b, 0).value;
    dataRegios.forEach(item => {
        let color = 255
        let fid = mapData.find(it => item.name == it.name)
        item.itemStyle.opacity = 1

        let colorM = 139
        if (fid) {
            color = color - color * (fid.value / maxValue)
            colorM = colorM - (colorM - colorM * (fid.value / maxValue))
        }

        item.itemStyle.areaColor = `rgb(${colorM},${color},${color})`
    })
    ...
    function formatterShow(params) {
        ...
        let arr = [data.name, "用户数量：" + data.value];
        return arr.join("\n")
    }

    var point = 'image://images/map/point.png';
    var curIndx = 0;
    ...
    function setVal() {
        ...
        timer = setInterval(function () {
            ...
            mapChart.dispatchAction({ type: 'showTip', seriesIndex: 0, dataIndex: mapChart.currentIndex })

            // 有数据
            // if (mapData[mapChart.currentIndex].label && mapData[mapChart.currentIndex].label.show) {
            //     mapChart.currentIndex = -1;
            //     monthIndex++;
